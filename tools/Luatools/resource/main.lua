...
local gps_data = {
    raw_lat = nil,       -- Түүхий өргөрөг (DDMM.MMMM)
    raw_lon = nil,       -- Түүхий уртраг (DDDMM.MMMM)
    latitude = nil,      -- Хөрвүүлсэн өргөрөг (DD.DDDDDD)
    longitude = nil,     -- Хөрвүүлсэн уртраг (DDD.DDDDDD)
    speed = nil,         -- Хурд (км/ц)
...
-- DDDMM.MMMM форматыг DD.DDDDDD формат руу хөрвүүлэх
...
function update_gps_data()
    ...
    if locStr then
        ...
        if #parts >= 2 then
            -- Түүхий утгуудыг хадгалах
            gps_data.raw_lat = parts[1]
            gps_data.raw_lon = parts[2]

            -- Координатыг хөрвүүлэх
            gps_data.latitude = convert_gps_coordinate(parts[1], false)  -- Өргөрөг
            gps_data.longitude = convert_gps_coordinate(parts[2], true)  -- Уртраг
...
-- Нэмэлт GPS мэдээлэл авах
function get_additional_gps_info()
    -- libgnss.getLocation ашиглан хурд, өндөр мэдээлэл авах
...
-- GPS мэдээлэл автоматаар шинэчлэх (5 секунд тутамд)
...
-- GPS мэдээллийг хэвлэх (15 секунд тутамд)
sys.timerLoopStart(function()
    print_gps_data()
end, 15000)
