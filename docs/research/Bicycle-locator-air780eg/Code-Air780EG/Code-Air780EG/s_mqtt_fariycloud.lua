...
sys.taskInit(function()
    ...
    -- 如果是超时,返回false,否则返回true


    if result then
        log.info("result 成功:",result)
    else
        log.info("result 失敗:",result)
        -- 5分钟后重试
        _G.deeprest_time = 5*60*1000
...
-- EC718P 是air780ep芯片
        ...
        _G.devicebsp = rtos.bsp()
        _G.coreversion = rtos.version()

        log.info("devicebsp:",devicebsp)
        log.info("coreversion:",coreversion)
        ...
        while SRCCID =="" do
            ...
            local str5 = "api/getDeviceByMac_appkey=" .. appkey .. "_coreversion=" .. coreversion ..  "_devicebsp=" .. devicebsp ..  "_did=" .. did ..  "_imei=" .. imei .. "_mac=" .. mac .."_nonce="..nonce.."_signt="..signt.."_version=".. version.."_".. secretkey;
            local str6 = string.urlEncode(str5)
            local sign =  string.lower (crypto.md5(str6,#str6))
            local str = string.format('%s?appkey=%s&coreversion=%s&devicebsp=%s&did=%s&imei=%s&mac=%s&nonce=%s&signt=%s&version=%s&sign=%s',server_api,appkey,coreversion,devicebsp,did,imei,mac,nonce,signt,version,sign)
            log.info("str:",str)
